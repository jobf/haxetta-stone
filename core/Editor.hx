...
class Editor {
	...
	// public function line_create(x_from:Float, y_from:Float, x_to:Float, y_to:Float):Void {}
}
...
class Designer {
	...
	function map_figure(model:Model):Figure {
		...
		return {
			model: model.lines,
			lines: graphics.model_to_lines(model.lines, 0xFFFFFFff)
		}
	}
	...
	public function line_under_cursor_remove() {
		...
		var models_under_cursor = figure.model.filter(model -> 
			model.from.x == line_under_cursor.point_from.x
			&& model.from.y == line_under_cursor.point_from.y
			&& model.to.x == line_under_cursor.point_to.x
			&& model.to.y == line_under_cursor.point_to.y);
